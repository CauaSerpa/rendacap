<?php
	function subsFreePlan($customer_id, $dataForm, $config, $conn) {
        ...
        $dataForm['payment_method'] = "FREE_PLAN";
        ...
        // Insere um novo registro na tabela tb_subscriptions
        $stmt = $conn->prepare("
            INSERT INTO tb_subscriptions (user_id, plan_id, customer_id, order_id, payment_id, cpf, cycle, price, payment_method, payment_status, status, creation_date, due_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([
            $dataForm['user_id'],
