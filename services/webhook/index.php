...
function processPaymentData($data, $conn) {
    ...
    $sql = "SELECT * FROM $tabela WHERE payment_id = :payment_id";
    ...
    if ($transacao) {
        ...
        // CREATE
        $sql = "INSERT INTO $tabela (creation_date, customer_id, subscription_id, price, net_value, description, billing_type, confirmed_date, credit_card_number, credit_card_brand, credit_card_token, status, credit_date, estimated_credit_date, webhook_date_created) VALUES (:event, :payment_id, :creation_date, :customer_id, :subscription_id, :price, :net_value, :description, :billing_type, :confirmed_date, :credit_card_number, :credit_card_brand, :credit_card_token, :status, :credit_date, :estimated_credit_date, :webhook_date_created)";
        $stmt = $conn->prepare($sql);
